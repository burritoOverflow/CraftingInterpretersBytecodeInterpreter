...
// local variables are compiled to direct slot access (the name never leaves the compiler)
// so, we'll just show the slot number
...
// used during disassembly for displaying the method called during `OP_INVOKE`
static int invokeInstruction(const char* name, Chunk* chunk, int offset) {
    const uint8_t constant = chunk->code[offset + 1];
    const uint8_t argCount = chunk->code[offset + 2];

    // display the number of args and the name of the method
...
int disassembleInstruction(Chunk* chunk, int offset) {
    ...
    switch (instruction) {
        ...
        // arithmetic operators take operands (from stack); bytecode instructions do
        // not
        case OP_SUBTRACT:
